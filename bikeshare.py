...
def get_filter_city():
    ...
    """
    #build and display cities related to our datasets
    cities_list = []
    num_cities = 0
...
def get_filter_month():
    ...
    while True:
        try:
            month = input("    Please Enter the month with January=1, June=6 or 'a' for all:  ")
        except:
...
def station_stats(data):
    ...
    filtered_rides = len(data)

    # display most commonly used start station
    start_station = data['Start Station'].mode()[0]
    start_station_trips = data['Start Station'].value_counts()[start_station]
    ...
    end_station_trips = data['End Station'].value_counts()[end_station]
